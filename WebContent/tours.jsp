<!DOCTYPE html>
<html>
    <head>
        <meta charset="UTF-8">
        <title>tours</title>
        <link rel="stylesheet" href="styles/style.css">
    </head>
    <body>
        <h1 class="titolo">TOUR DISPONIBILI</h1>
        <div class="card-container">
            <div class="card">
                <img class="card-img" src="/AniTour/images/persona5.jpg" alt="persona5">
                <div class="card-title">
                    <a class="card-link" href="persona5.html">persona 5</a>
                </div>
                <div class="card-text">
                    Maschere calate. Cuori da rubare. Ribellione tra ombre e città lucenti.<br>
                    Dal 24 giugno al 7 luglio.<br>
                    Infiltrati nei Palazzi dell’ordinario, risveglia il tuo io!<br>
                    Questo e molto altro, nel tour a tema Persona 5.
                </div>
            </div>
            <a href="bloodborne.html" class="card">
                <img class="card-img" src="/AniTour/images/bloodborne.jpg" alt="bloodborne">
                <div class="card-title">
                    <div class="card-link">bloodborne</a>
                </div> 
                <div class="card-text">Nebbia eterna. Campane lontane. Pietra, sangue e sogni spezzati.<br>
                    Questo ed altro nel tour a tema Bloodborne, dal 12 al 19 febbraio.<br>
                </div>

            </div>
            <div class="card">
                <img class="card-img" src="/AniTour/images/sekiro.jpg" alt="sekiro">
                <div class="card-title">
                    <a class="card-link" href="sekiro.html">sekiro</a>
                </div>
                <div class="card-text">
                    Lama affilata. Ombre furtive. Onore, vendetta e redenzione.<br>
                    Dal 5 al 18 settembre.<br>
                    Tra castelli antichi e spiriti inquieti, cammina la via del lupo solitario.<br>
                    Questo e molto altro, nel tour a tema Sekiro
                </div>
            </div>
            <div class="card">
                <img class="card-img" src="/AniTour/images/outerwilds.jpg" alt="outerwilds">
                <div class="card-title">
                    <a class="card-link" href="outerwilds.html">outer wilds</a>
                </div>
                <div class="card-text"> 
                    Orbite perdute. Segreti antichi. Il tempo si piega, il mistero chiama.<br>
                    Dal 10 al 23 agosto.<br>
                    Naviga tra stelle e ricordi, scopri ci&ograve; che l’universo nasconde.<br>
                    Questo e molto altro, nel tour a tema Outer Wilds.
                </div>
            </div>
            <div class="card">
                <img class="card-img" src="/AniTour/images/kon.jpg" alt="kon">
                <div class="card-title">
                    <a class="card-link" href="kon.html">k-on</a>
                </div>
                <div class="card-text">
                    Note leggere. Risate sincere. T&egrave;, chitarre e amicizie che brillano.<br>
                    Dal 1 giugno al 14 luglio.<br>
                    Dalla scuola al palco, ogni giorno &egrave; un piccolo concerto.<br>
                    Questo e molto altro, nel tour a tema K-ON!
                </div>
            </div>
            <div class="card">
                <img class="card-img" src="/AniTour/images/vinland saga.jpg" alt="vinland">
                <div class="card-title">
                    <a class="card-link" href="vinlandsaga.html">vinland saga</a>
                </div>
                <div class="card-text">
                    Vento del nord. Lame sporche di fango e destino. Onore, vendetta, libert&agrave;.<br>
                    Dal 1° al 14 luglio.<br>
                    Dalle coste gelide alla terra promessa, ogni passo &egrave; guerra e rinascita.<br>
                    Questo e molto altro, nel tour a tema Vinland Saga!
                </div>            
        </div>
    </body>
</html>